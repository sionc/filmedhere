require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe FilmsController do

  # This should return the minimal set of attributes required to create a valid
  # Film. As you add validations to Film, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) { { "title" => "MyString" } }

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # FilmsController. Be sure to keep this updated too.
  let(:valid_session) { {} }

  describe "GET index" do
    it "assigns all films as @films" do
      film = Film.create! valid_attributes
      get :index, {}, valid_session
      expect(assigns(:films)).to eq([film])
    end
  end

  describe "GET show" do
    it "assigns the requested film as @film" do
      film = Film.create! valid_attributes
      get :show, {:id => film.to_param}, valid_session
      expect(assigns(:film)).to eq(film)
    end
  end

  describe "GET new" do
    it "assigns a new film as @film" do
      get :new, {}, valid_session
      expect(assigns(:film)).to be_a_new(Film)
    end
  end

  describe "GET edit" do
    it "assigns the requested film as @film" do
      film = Film.create! valid_attributes
      get :edit, {:id => film.to_param}, valid_session
      expect(assigns(:film)).to eq(film)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Film" do
        expect {
          post :create, {:film => valid_attributes}, valid_session
        }.to change(Film, :count).by(1)
      end

      it "assigns a newly created film as @film" do
        post :create, {:film => valid_attributes}, valid_session
        expect(assigns(:film)).to be_a(Film)
        expect(assigns(:film)).to be_persisted
      end

      it "redirects to the created film" do
        post :create, {:film => valid_attributes}, valid_session
        expect(response).to redirect_to(Film.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved film as @film" do
        # Trigger the behavior that occurs when invalid params are submitted
        Film.any_instance.stub(:save).and_return(false)
        post :create, {:film => { "title" => "invalid value" }}, valid_session
        expect(assigns(:film)).to be_a_new(Film)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Film.any_instance.stub(:save).and_return(false)
        post :create, {:film => { "title" => "invalid value" }}, valid_session
        expect(response).to render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested film" do
        film = Film.create! valid_attributes
        # Assuming there are no other films in the database, this
        # specifies that the Film created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        expect_any_instance_of(Film).to receive(:update).with({ "title" => "MyString" })
        put :update, {:id => film.to_param, :film => { "title" => "MyString" }}, valid_session
      end

      it "assigns the requested film as @film" do
        film = Film.create! valid_attributes
        put :update, {:id => film.to_param, :film => valid_attributes}, valid_session
        expect(assigns(:film)).to eq(film)
      end

      it "redirects to the film" do
        film = Film.create! valid_attributes
        put :update, {:id => film.to_param, :film => valid_attributes}, valid_session
        expect(response).to redirect_to(film)
      end
    end

    describe "with invalid params" do
      it "assigns the film as @film" do
        film = Film.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Film.any_instance.stub(:save).and_return(false)
        put :update, {:id => film.to_param, :film => { "title" => "invalid value" }}, valid_session
        expect(assigns(:film)).to eq(film)
      end

      it "re-renders the 'edit' template" do
        film = Film.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Film.any_instance.stub(:save).and_return(false)
        put :update, {:id => film.to_param, :film => { "title" => "invalid value" }}, valid_session
        expect(response).to render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested film" do
      film = Film.create! valid_attributes
      expect {
        delete :destroy, {:id => film.to_param}, valid_session
      }.to change(Film, :count).by(-1)
    end

    it "redirects to the films list" do
      film = Film.create! valid_attributes
      delete :destroy, {:id => film.to_param}, valid_session
      expect(response).to redirect_to(films_url)
    end
  end

end
